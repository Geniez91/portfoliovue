...
export class AuthService {
  ...
  async signIn(email: string, password: string): Promise<IAccessToken> {
    ...
    if (error) {
      this.logger.error(
        `${ELoggerContext.AuthService.signIn} with email : ${email} and password : ${password} and error ${error}`,
      );
      throw new UnauthorizedException();
    }
    const payload = { sub: data.user.id, email: data.user.email };
    this.logger.debug(
      `${ELoggerContext.AuthService.signIn} with email : ${email} and password : ${password}`,
    );
