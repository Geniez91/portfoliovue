  ...
  <v-dialog activator="parent" max-width="500">
    <v-card rounded="lg">
      <v-card-title class="d-flex justify-space-between align-center">
        <div class="text-h5 text-medium-emphasis ps-2">Ajouter une Compétence</div>

        <v-btn icon="mdi-close" variant="text" @click="emit('add-dialog', false)"></v-btn>
      </v-card-title>

      <v-divider class="mb-4"></v-divider>

      <v-card-text>
        <v-alert v-if="errorMessage" class="mb-5" color="error" icon="mdi-check-circle">{{
          errorMessage
        }}</v-alert>
        <div>
          <div class="text-medium-emphasis mb-4">Nom de la compétence :</div>
          <v-text-field
            v-model="nameSkills"
            @update:model-value="nameSkills = $event"
          ></v-text-field>
        </div>
        <div>
          <div class="text-medium-emphasis mb-4">Type de Compétence</div>
        ...
        <div v-if="selectTypeSkills !== 'Soft Skills' && selectTypeSkills !== 'Languages'">
          <div class="text-medium-emphasis mb-4">Année d'expérience</div>
        ...
        <div v-if="selectTypeSkills !== 'Soft Skills' && selectTypeSkills !== 'Languages'">
          <div class="text-medium-emphasis mb-4">Dernière utilisation</div>
          <v-date-input
        ...
        <div v-if="selectTypeSkills === 'Languages'">
          <div class="text-medium-emphasis mb-4">Niveau :</div>
          <v-select
            :items="skillsLevel"
            :model-value="selectSkillsLevel"
            @update:model-value="selectSkillsLevel = $event"
          ></v-select>
        </div>
        <div v-if="selectTypeSkills === 'Languages'">
          <div class="text-medium-emphasis mb-4">TOIEC :</div>
          <v-number-input
            :reverse="false"
            controlVariant="split"
            label=""
        ...
        <div>
          <div class="text-medium-emphasis mb-4">Image d'illustration de la compétence :</div>
...
async function addSkillsEvent(
  nameSkills: string,
  selectTypeSkills: TSkills,
  imageSkills: File,
  token: string,
