  ...
  <v-dialog v-if="dialogDelete" activator="parent" max-width="500">
    <v-card rounded="lg">
      <v-card-title class="d-flex justify-space-between align-center">
        <div class="text-h5 text-medium-emphasis ps-2">Supprimer une Compétence</div>

        <v-btn icon="mdi-close" variant="text" @click="emit('delete-dialog', false)"></v-btn>
      </v-card-title>
      ...
      <v-card-text>
        <div>
          <p>
            Etes-vous sur de vouloir supprimer la compétence {{ props.selectedLanguage }} de votre
            portfolio ?
          </p>
        </div>
      </v-card-text>
