  ...
  <v-dialog activator="parent" max-width="500">
    <v-card rounded="lg">
      <v-card-title class="d-flex justify-space-between align-center">
        <div class="text-h5 text-medium-emphasis ps-2">Modifier une Compétence</div>

        <v-btn icon="mdi-close" variant="text" @click="emit('update-dialog', false)"></v-btn>
      </v-card-title>
      ...
      <v-card-text>
        <div>
          ...
          <v-text-field
            ...
            @update:model-value="skill.language = $event"
          ></v-text-field>
        </div>
        <div>
        ...
        <div v-if="skill.idType === 'Languages'">
          ...
          <v-select
            ...
            @update:model-value="skill.level = $event"
          ></v-select>
        </div>
        <div v-if="skill.idType === 'Languages'">
