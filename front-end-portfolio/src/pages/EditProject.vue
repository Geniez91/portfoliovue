  ...
  <div class="mx-3 my-5">
      ...
      <v-form>
        <div class="my-2">
          ...
          <v-text-field
            ...
            @update:model-value="currentProject.name = $event"
            :rules="[emptyRules]"
          >
          </v-text-field>
        </div>
        <div class="my-2">
          <p>Description :</p>
          <v-text-field type="text" v-model="currentProject.description" :rules="[emptyRules]">
          </v-text-field>
        </div>

        <div class="my-2">
          <p>Lien Github :</p>
          <v-text-field type="text" v-model="currentProject.linkGithub" :rules="[emptyRules]">
          </v-text-field>
        </div>

        <div class="my-2">
          <p>Nombre de collaborateur :</p>
          <v-number-input v-model="currentProject.nbCollaborator"></v-number-input>
        </div>
    ...
    <div>
      ...
      <div class="d-flex justify-center">
        <v-btn color="info" prepend-icon="mdi-plus" class="my-5" @click="updateProjectHandler"
          >Editer</v-btn
        >
      </div>
    </div>
...
function findWorkExperience() {
  const found = projects.value.find((exp) => exp.id?.toString() === route.params.id)
  if (!found) {
...
async function updateProjectHandler() {
  ...
  if (token.value) {
    ...
    await projectExperienceStore.updateProjects(
      token.value,
