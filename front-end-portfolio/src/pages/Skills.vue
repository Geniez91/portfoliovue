<template>
  <div class="ml-3">
    <div v-if="succesMessage" class="my-5">
      <v-alert color="success" icon="mdi-check-circle" width="fit-content">{{
        succesMessage
      }}</v-alert>
    </div>
    <div class="mt-5">
    ...
    <div class="mt-3" :class="smAndDown ? 'd-flex flex-column' : 'd-flex'">
      <v-btn
        :class="smAndDown ? 'mb-3' : ''"
        v-for="skills in skillstabs"
        class="mr-3"
        color="white"
        :active="skillsShow === skills"
        active-color="secondary"
        :style="{ border: skillsShow !== skills ? '2px solid' : '' }"
        @click="changeSkillsMode(skills as TSkillsShow)"
        >{{ skills }}</v-btn
      >
    </div>
  </div>
  ...
  <DialogUpdate
    @success-message="succesMessage = $event"
    @error-message="errorMessage = $event"
    :skill="selectedSkills"
    :dialog-update="dialogUpdate"
    @update-dialog="dialogUpdate = $event"
    v-if="dialogUpdate"
  ></DialogUpdate>
  <DialogDelete
    @success-message="succesMessage = $event"
    @error-message="errorMessage = $event"
    :id-langage="idLangage"
    :dialog-delete="dialogDelete"
    :selected-language="selectedLangage"
    @delete-dialog="dialogDelete = $event"
    v-if="dialogDelete"
  ></DialogDelete>
  <DialogAdd
    v-if="dialogAdd"
  ...
  <div v-if="skillsShow === 'Développement Web'">
    <div class="ml-3">
      ...
      <SkillsCard
        ...
        :is-loaded="isLoaded"
        :type-language="getSkillsByType('Front-end')"
    ...
    <div class="mt-5 ml-3">
      ...
      <div class="d-flex mb-5" style="width: fit-content">
        <SkillsCard
          ...
          @delete-skill="deleteSkill"
          @update-skill="updateSkill"
          :type-language="getSkillsByType('Back-end')"
        ></SkillsCard>
      </div>
    </div>
  </div>
    ...
    <div class="ml-3">
      ...
      <div class="d-flex mb-5" style="width: fit-content">
        <SkillsCard
          ...
          @delete-skill="deleteSkill"
          @update-skill="updateSkill"
          :type-language="getSkillsByType('Base de données')"
        ></SkillsCard>
      </div>
    </div>
    <v-divider class="border-opacity-75 mt-12" color="primary"></v-divider>
    <div class="ml-3 mt-5">
      ...
      <div class="d-flex mb-5" style="width: fit-content">
        <SkillsCard
          ...
          @delete-skill="deleteSkill"
          @update-skill="updateSkill"
          :type-language="getSkillsByType('Modelisation')"
        ></SkillsCard>
      </div>
    </div>
  </div>
    ...
    <div class="ml-3 mt-5">
      ...
      <div class="d-flex mb-5" style="width: fit-content">
        <SkillsCard
          ...
          @delete-skill="deleteSkill"
          @update-skill="updateSkill"
          :type-language="getSkillsByType('Soft Skills')"
          :without-card="true"
        ></SkillsCard>
      </div>
    </div>
  ...
  <div v-if="skillsShow === 'Langues'">
    <div class="ml-3 mt-5">
      ...
      <LanguageExperience
        ...
        @update-skill="updateSkill"
        @delete-skill="deleteSkill"
        :language-experience="getSkillsByType('Languages')"
      ></LanguageExperience>
    </div>
  </div>
