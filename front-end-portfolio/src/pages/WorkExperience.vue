  ...
  <div class="mx-3 my-5">
    ...
    <div v-if="workExperienceStore.successMessage" class="my-5">
      <v-alert color="success" icon="mdi-check-circle" width="fit-content">{{
        workExperienceStore.successMessage
      }}</v-alert>
    </div>
    <div class="mt-7">
      <v-row style="width: fit-content">
        <v-col v-for="experience in workExperiences" cols="12" :key="experience.id">
          <div class="mb-3">
            <p class="text-primary text-body-2" style="font-style: italic">
              {{ formatDateToMonthYear(experience.startDate) }} -
              {{ formatDateToMonthYear(experience.endDate) }}
              {{ calculateDifferenceBetweenTwoDates(experience.startDate, experience.endDate) }}
            </p>
          </div>
          ...
          <v-card color="secondary" hover :to="`workExperience/${experience.nameCompany}`" v-else>
            <div class="d-flex">
              <v-img
                :src="experience.srcImg as string"
                :alt="`${experience.nameCompany} logo`"
                style="border: 1px solid; border-radius: 5px; width: 70px"
              ></v-img>
              <div class="d-flex flex-column justify-center ml-3">
                <p class="mb-2 text-primary font-weight-bold text-subtitle-1">
                  {{ experience.job }}
                </p>
            ...
            <div class="ml-3 mt-2">
              <p class="text-primary text-subtitle-1" style="text-decoration: underline">
                Taches Effectués :
              </p>
              <ul>
            ...
            <div class="d-flex flex-column align-center my-2">
              <router-link :to="`workExperience/${experience.nameCompany}`"
                ><v-btn color="navbar">En savoir plus ?</v-btn></router-link
              >
            </div>
          </v-card>
