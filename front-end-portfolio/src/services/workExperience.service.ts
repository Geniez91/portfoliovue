...
const urlDeleteWorkExperience =
  'https://portfoliovue-back-end-production.up.railway.app/workExperience?id='
...
export async function deleteWorkExperience(token: string, id: number) {
  try {
    const result = await axios.delete(`${urlDeleteWorkExperience}${id}`, {
...
export async function updateWorkExperience(workExperience: IWorkExperience2, token: string) {
  try {
    ...
    const result = await axios.put(`${urlDeleteWorkExperience}${workExperience.id}`, formData, {
      headers: addHeaders(token),
    })
    return result
  } catch (error) {
